...
class Unet(nn.Module):
    def __init__(
        ...
        if attn_blocks > 1:
            self.mid_attn = nn.ModuleList([Residual(FullAttention(mid_dim, dim_head = attn_dim_head[-1], heads = attn_heads[-1])) for _ in range(attn_blocks)])
            # Pack it as a single calling function
            self.mid_attn = nn.Sequential(*self.mid_attn)
        else:
...
class Trainer(object):
    def __init__(
        ...
        sampler = RandomSampler(self.ds, replacement=True, num_samples=train_num_steps * train_batch_size)
        dl = DataLoader(self.ds, batch_size=train_batch_size, sampler=sampler, pin_memory=True, num_workers=cpu_count())
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.see_every):
                        self.ema.ema_model.eval()

                        with torch.inference_mode():
                            milestone = self.step // self.see_every
